...
class PackScreen extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    ...
    final ink = over ? Ink_.amber : (kg > 10.8 ? Ink_.blue : Ink_.green);
      ...
      children: [
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              Panel(
                bg: over ? Ink_.bgWarn : Ink_.surface,
                borderColor: over ? Ink_.borderWarn : Ink_.border,
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text(fmtDec(kg), style: T.num(size: 44, color: ink)),
                        const SizedBox(width: 5),
                        Padding(
                          padding: const EdgeInsets.only(bottom: 7),
                          child: Lbl('KG', color: Ink_.text2, size: 13),
                        ),
                        const Spacer(),
                        Lbl('LIMITE 12 KG', color: Ink_.dim, size: 8),
                      ],
                    ),
                    const SizedBox(height: 11),
                    Bar((kg / 15).clamp(0, 1), color: ink, height: 4),
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                            child: Stat('ENERGIA', '${fmtMil(kcal(s.gramas))} kcal',
                                size: 16)),
                        Expanded(
                          child: Stat('CHEGADA', s.arrive,
                              size: 16,
                              color: over || s.dark ? Ink_.amber : Ink_.textStrong),
                        ),
                      ],
                    ),
                    const SizedBox(height: 14),
                    Text(veredito,
                        style: T.body(size: 10, color: over ? Ink_.amber : Ink_.text2)),
                  ],
                ),
              ),
                ...
                child: Row(
                  children: [
                    Icon(Icons.route_outlined,
                        size: 14,
                        color: s.core.variants.isEmpty ? Ink_.green : Ink_.amber),
              ...
              const SizedBox(height: 14),
              const Lbl('FIXOS · NÃO SAEM DA MOCHILA', size: 9, tracking: 0.26),
              const SizedBox(height: 8),
              ...packData.where((d) => d.fixo).map((d) => _ItemRow(item: d, fixo: true)),
              const SizedBox(height: 14),
              const Lbl('ESCOLHA · TOQUE PARA DEIXAR EM CASA',
                  size: 9, tracking: 0.26),
              const SizedBox(height: 8),
              ...packData.where((d) => !d.fixo).map((d) => _ItemRow(item: d, fixo: false)),
              const SizedBox(height: 6),
              Btn(over ? 'SEGUIR ASSIM MESMO' : 'CONFIRMAR CARGA',
                  onTap: () => s.go('route'),
                  primary: !over,
                  bg: over ? Ink_.bgWarn : null,
                  border: over ? Ink_.borderWarn : null,
                  ink: over ? Ink_.amber : null),
            ],
          ),
        ),
      ],
  ...
  Widget build(BuildContext c) {
    ...
    return Padding(
      ...
      child: Panel(
        ...
        bg: fora ? Ink_.surfaceAlt : Ink_.surface,
        borderColor: fora && exigido ? Ink_.borderWarn : Ink_.border,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(
              children: [
                Icon(
                  ...
                  size: 14,
                  color: fixo ? Ink_.dim : (fora ? Ink_.dim : Ink_.green),
                ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        ...
                        style: T.label(
                            size: 11,
                            color: fora ? Ink_.dim : Ink_.text,
                            tracking: 0.12).copyWith(
                          decoration: fora ? TextDecoration.lineThrough : null,
                          decorationColor: Ink_.dim,
                        ),
                      ),
                      const SizedBox(height: 3),
                      Text(item.note, style: T.body(size: 9, color: Ink_.dim)),
                    ],
                  ),
                ),
                ...
                Text('${fmtMil(item.gramas)} g',
                    style: T.num(
                        size: 13,
                        color: fora ? Ink_.dim : Ink_.text2,
            ...
            if (fora && item.consequencia.isNotEmpty) ...[
              ...
              Text(item.consequencia,
                  style: T.body(size: 9, color: exigido ? Ink_.amber : Ink_.text3)),
            ],
            if (fora && exigido) ...[
              const SizedBox(height: 7),
              Lbl('A PARTIDA ESCOLHIDA EXIGE ESTE ITEM',
                  color: Ink_.amber, size: 8, tracking: 0.14),
            ],
          ],
        ),
      ),
