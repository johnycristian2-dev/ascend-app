      ...
      children: [
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              const AppLabel('CONVIDADOS', size: 9, tracking: 0.26),
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            border: Border.all(color: on ? AppColors.green : AppColors.border),
                            shape: BoxShape.circle,
                          ),
                          child: AppLabel(p.$3,
                              color: on ? AppColors.green : AppColors.dim, size: 9, tracking: 0.1),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              AppLabel(p.$2, color: on ? AppColors.text : AppColors.text3, size: 11),
                              const SizedBox(height: 3),
                              Text(p.$4, style: AppTypography.body(size: 9, color: AppColors.dim)),
                            ],
                          ),
                        ),
                        Icon(on ? Icons.check_circle_outline : Icons.circle_outlined,
                            size: 15, color: on ? AppColors.green : AppColors.dim),
                      ],
                    ),
                  ),
                );
              ...
              const AppLabel('EQUIPAMENTO COLETIVO · QUEM LEVA', size: 9, tracking: 0.26),
              const SizedBox(height: 8),
              AppPanel(
                borderColor: missing > 0 ? AppColors.borderWarn : AppColors.border,
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ..._gear.map((g) {
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                  child: AppLabel(g.$2,
                                      color: on ? AppColors.text : AppColors.dim, size: 10)),
                              Text(g.$3,
                                  style: AppTypography.num(
                                      size: 12,
                                      color: on ? AppColors.text2 : AppColors.dim,
                    ...
                    Row(
                      ...
                      children: [
                        const AppLabel('CARGA COLETIVA', color: AppColors.dim, size: 9),
                        Text('${total.toStringAsFixed(1).replaceAll('.', ',')} kg',
                            style: AppTypography.num(size: 15)),
                      ],
                    ),
                    if (missing > 0) ...[
                      const SizedBox(height: 10),
              ...
              AppButton('ENVIAR CONVOCAÇÃO', onTap: () => s.go('chat'), primary: true),
            ],
          ),
        ),
      ],
